...
class Setpoint:
    ...
    def reached(self, topic):
            ...
            if abs(topic.pose.position.x - self.x) < 1.5 and abs(topic.pose.position.y - self.y) < 1.5 and abs(topic.pose.position.z - self.z) < 0.75:
                self.done = True
...
#    setpoint.set(0.0, 0.0, 4.0, 5)
    ...
    print "Fly in a sine curve"
    
    tot_deg = 360
    deg = 0
    height = 4.0
    setpoint.set(0.0, 0.0, height, 5, True)   # Go To Initial Reference Point
    
    while not rospy.is_shutdown():
        x = deg*2*pi/tot_deg
        y = 3*sin(deg*2*pi/tot_deg)
        
        if (deg == 0 or deg == tot_deg):
            setpoint.set(x, y, height, 5, True)            
        else:
            setpoint.set(x, y, height, 0, False)            
        
        deg = deg + 1
        rate.sleep()

        if (deg > tot_deg):
            print "Offboard Control Over"
            break
